// Generated by using Rcpp::compileAttributes() -> do not edit by hand
// Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

#include <Rcpp.h>

using namespace Rcpp;

#ifdef RCPP_USE_GLOBAL_ROSTREAM
Rcpp::Rostream<true>&  Rcpp::Rcout = Rcpp::Rcpp_cout_get();
Rcpp::Rostream<false>& Rcpp::Rcerr = Rcpp::Rcpp_cerr_get();
#endif

// jarvis_march_cpp
std::vector<std::vector<double>> jarvis_march_cpp(const std::vector<std::vector<double>>& xy);
RcppExport SEXP _convexhull_jarvis_march_cpp(SEXP xySEXP) {
BEGIN_RCPP
    Rcpp::RObject rcpp_result_gen;
    Rcpp::RNGScope rcpp_rngScope_gen;
    Rcpp::traits::input_parameter< const std::vector<std::vector<double>>& >::type xy(xySEXP);
    rcpp_result_gen = Rcpp::wrap(jarvis_march_cpp(xy));
    return rcpp_result_gen;
END_RCPP
}

static const R_CallMethodDef CallEntries[] = {
    {"_convexhull_jarvis_march_cpp", (DL_FUNC) &_convexhull_jarvis_march_cpp, 1},
    {NULL, NULL, 0}
};

RcppExport void R_init_convexhull(DllInfo *dll) {
    R_registerRoutines(dll, NULL, CallEntries, NULL, NULL);
    R_useDynamicSymbols(dll, FALSE);
}
